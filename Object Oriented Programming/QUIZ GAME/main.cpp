...
int num_quizzes=0;
int arr_score [10]={999,999,999,999,999,999,999,999,999,999};
float Total_Average=0.0;
float MCQ_sum=0.0;
float TF_sum=0.0;
float Complete_sum=0.0;
            ...
            if(que_type == "1"){
                ...
                for (int i = 0; i < 5; i++) {
                    ...
                    if (i == 0){
                        ...
                        if (i == 1) MCQ_questions[MCQ_capacity - 1].setAns(data); /// set the right answer
            ...
            if (all_question_capacity > 0) {
                ...
                for (int i = 0; i < MCQ_capacity; i++) {
                    ...
                    for(int j=0; j < 4; j++){
                        ...
                        if(j == 0) tempChoice += "*"; /// to show to the admin the right answer
...
void admin_menu(User x){
    ...
    else if (myChoice == "2") root.update_name();
...
void player_menu(User a){
    ...
    else if (myChoice == "2") {plyr.update_name();}
    else if (myChoice == "3") {plyr.newQuiz(a);}
    else if (myChoice == "4") {plyr.DISPLAY_SCORE_STATISTICS();}
    else if (myChoice == "5") {plyr.DISPLAY_ALL_SCORES();}
...
cout << "************************************************"  << "\n";
cout << "my grade is equal " << (correct_MCQ*1+correct_TF*2+correct_complete*3)<< " from 10"<< "\n";
cout << "Wrong answer : " << questions_per_quiz * 2 - (correct_MCQ*1+correct_TF*2+correct_complete*3)<< "\n";
cout << "************************************************"  << "\n";



arr_score[num_quizzes] = (correct_MCQ*1+correct_TF*2+correct_complete*3);
Total_Average+=arr_score[num_quizzes];

MCQ_sum+=correct_MCQ;
TF_sum+=correct_TF;
Complete_sum+=correct_complete;
...
void Quiz::DISPLAY_SCORE_STATISTICS()
{
if(num_quizzes<1)
{
    ...
    cout << "**********************************************************************************" << "\n";
    cout << "Wrong!!"<<"\n";
    cout << "You should take one quiz at least."<< "\n";
    cout << "**********************************************************************************" << "\n";
}
else
{
sort(arr_score,arr_score+10);

cout << "Your score statistics per quiz:" << "\n";
cout << "           -Number of Quizzes taken: "<< num_quizzes      << "\n";
cout << "           -Highest quiz score: "<<arr_score[num_quizzes-1]  << "/10" << "\n";
cout << "           -Lowest quiz score: "<<arr_score[0]               << "/10" << "\n";
cout << "           -Average quiz score: "<<setprecision(2)<< Total_Average/num_quizzes       <<"/10"  <<"\n";


cout << "Your score statistics per question type:"<< "\n" ;
cout << "           - Number of MC questions: " << cnt_MCQ*num_quizzes << "\n";
cout << "           - Number of Complete questions: "<< cnt_Complete*num_quizzes<< "\n";
cout << "           - Number of T/F Questions: " << cnt_TF* num_quizzes << "\n";
cout << "           - Average grade for MC questions: "<<setprecision(2)<<(MCQ_sum*1)/(cnt_MCQ*num_quizzes) << "\n";
cout << "           - Average grade for Complete questions: " <<setprecision(2)<<(Complete_sum*2)/(cnt_Complete*num_quizzes) << "\n";
cout << "           - Average grade for T/F questions: " <<setprecision(2)<<(TF_sum*3)/(cnt_TF*num_quizzes)<<"\n";
cout << "\n\n";
...
cout << "correct_MCQ = " <<MCQ_sum<<"\n";
cout << "correct_TF = " <<TF_sum<<"\n";
cout << "correct_complete = " <<Complete_sum<<"\n";

cout << "\n\n";



cout << "Total questions = " << (cnt_MCQ +cnt_TF+cnt_Complete) * num_quizzes  << "\n" ;
cout << "************************************************"  << "\n";
cout << "total  grade :  " << (MCQ_sum*1+TF_sum*2+Complete_sum*3) <<  "\n";
cout << "total  Wrong  :  " << questions_per_quiz * 2 * num_quizzes  -   (MCQ_sum*1+TF_sum*2+Complete_sum*3)<<num_quizzes << "\n";
cout << "************************************************"  << "\n";
